...
import type { RedTeamEnvironmentReport, RedTeamReportTable } from './redteam.js';
...
function tables(r: RedTeamEnvironmentReport): RedTeamReportTable[] {
  return [
    ...r.tables,
    {
...
export function renderRedTeamReportMarkdown(r: RedTeamEnvironmentReport): string {
  const lines = [
    `# ${md(r.title)}`,
    '',
    `**${r.product}**`,
    ...
    '',
    `Scan creation window (UTC): ${r.window.start} → ${r.window.end}`,
    '',
  ...
  lines.push(
    ...
    '',
    `Prisma AIRS CLI · Red Team report schema ${r.schemaVersion}`,
    '',
  );
  return lines.join('\n');
}
...
export function renderRedTeamReportHtml(r: RedTeamEnvironmentReport): string {
