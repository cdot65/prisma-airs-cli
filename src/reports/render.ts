...
function tables(report: RuntimeDailyReport): Table[] {
  const sessionSource = report.sources.find((source) => source.name === 'Daily session inventory');
  return [
    {
    ...
    {
      title: 'Collected session observations',
      id: 'sessions',
      headers: ['Session status', 'Entries'],
      rows: report.sessions.statuses.map((entry) => [entry.name, number(entry.count)]),
      note: `Source: ${sessionSource?.status ?? 'unavailable'}. Timestamp-eligible entries: ${number(report.sessions.entries)}. Violated status: ${number(report.sessions.violatingSessions)}. Missing timestamps: ${report.sessions.missingTimestamps}; outside window: ${report.sessions.outsideWindow}. These are collected session counts, not scan actions or detector events. No scan content is fetched.`,
    },
    {
      title: 'Daily session chart',
      id: 'session-chart',
      headers: ['Bucket time (UTC)', 'Sessions', 'Violating sessions', 'Detector violations'],
      rows: report.dailyTelemetry.chart.buckets.map((bucket) => [
        bucket.time,
        number(bucket.sessions),
        number(bucket.violatingSessions),
        number(bucket.violations.total),
      ]),
      note: `Independent chart totals: ${number(report.dailyTelemetry.chart.sessions)} sessions; ${number(report.dailyTelemetry.chart.violatingSessions)} violating sessions. Do not force these counters to equal a separately paginated inventory.`,
    },
    {
      title: 'Top applications by detector violations',
      id: 'top-violations',
      headers: ['Application bucket', 'Detector violations', 'Detection types'],
      rows: report.dailyTelemetry.topApplications.map((app) => [
        app.name,
        number(app.violations),
        app.detectors.map((detector) => `${detector.name}: ${number(detector.count)}`).join(', '),
      ]),
      note: 'Server-ranked subset for one day, not a complete application inventory. Detector violations may exceed the number of violating sessions.',
    },
    {
      title: 'Daily detector severity trend',
      id: 'violation-trend',
      headers: ['Bucket time (UTC)', 'Critical', 'High', 'Medium', 'Low', 'Total'],
      rows: report.dailyTelemetry.violationTrend.map((bucket) => [
        bucket.time,
        ...(['critical', 'high', 'medium', 'low', 'total'] as const).map((key) =>
          number(bucket.violations[key]),
        ),
      ]),
      note: 'API-reported detector-policy events over the rolling day. Severity counts are preserved independently from distinct session counts; no previous-day comparison is inferred.',
    },
    {
...
export function renderRuntimeReportMarkdown(report: RuntimeDailyReport): string {
  ...
  lines.push(
    ...
    '',
    `Generated by Prisma AIRS CLI · Report schema ${report.schemaVersion}`,
    '',
  );
  return lines.join('\n');
}
...
${sections}<section id="scope"><h2>Scope, privacy, and limitations</h2><ul>${report.limitations.map((note) => `<li>${html(note)}</li>`).join('')}</ul></section><footer>Generated by Prisma AIRS CLI · Report schema ${report.schemaVersion} · Self-contained HTML; no network access required. All evidence remains readable with JavaScript disabled.</footer></main><script>${JS}</script></body></html>\n`;
}
