...
`--defaults <json>`, `--usage-limits <json>`, and `--rate-limits <json>`. Create additionally
requires `--name` and `--scope-name` and accepts `--users <ids>`; update is a partial patch and
requires at least one write flag.
...
fields. The CLI accepts a UUID or display name for `--workspace` and resolves it to the required
slug.

**Auth:** Management API credentials plus the appropriate AI Gateway SCM grant
...
`--variables` is a JSON array of `{ "key", "value", "type": "VAR"|"SECRET" }` objects.

**Important adapter rules:**

- Create/update accept exactly one of `--script-file` or `--script-b64` when supplying a script.
- Update is read-modify-write over an upstream full-replacement PUT. Stored variables are preserved;
  passing `--variables` replaces the complete set, so include every key to retain.
- Secret values render as `(redacted)`. On update, `value: null` for a `SECRET` keeps its stored
  value; never treat the API mask (`**********`) as a usable secret.
- Validation does not save an adapter. It requires an ONLINE network-broker channel and the full
  variable array. `--adapter <uuid>` can load the stored variable set and resolve stored secrets.
- Failed validation prints `stderr`/`traceback` and exits 1.
...
| AI Gateway `403` | Missing workspace-scope or tenant-root admin grant | Use data-plane commands only for assigned scopes, or obtain the SCM admin grant named in the CLI hint |
| Adapter channel is not ONLINE | Network broker is disconnected or still starting | Check `redteam network-broker channels list`; validate only through an ONLINE channel |
