...
vi.mock('../../../src/airs/management.js', async (original) => ({
  ...(await original<typeof import('../../../src/airs/management.js')>()),
  getOrCreateManagementClient: factory.getOrCreateManagementClient,
}));
...
describe('airs runtime report command', () => {
  ...
  it('writes HTML by default despite a general JSON output preference', async () => {
    ...
    expect(factory.getOrCreateManagementClient.mock.calls[0][0]).toMatchObject({ numRetries: 0 });
  });
  ...
  it('writes Markdown and honors explicit global and local artifact format flags', async () => {
    ...
    expect(await readFile(first, 'utf8')).toMatch(/^# Daily/);
  ...
  it('emits only the artifact on stdout and propagates stream errors', async () => {
    ...
    expect(write.mock.calls[0][0]).toMatch(/^# Daily/);
  ...
  it('delivers unique default HTML and Markdown files in the current working directory', async () => {
    ...
    const files = (await readdir(directory)).filter((name) =>
      name.startsWith('airs-runtime-report-'),
    );
    ...
    expect(await readFile(join(directory, markdownName ?? ''), 'utf8')).toMatch(/^# Daily/);
  });
