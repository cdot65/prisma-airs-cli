...
The **0.1.2** harness release includes CLI **7.1.5** and the updated
...
Restart `airs` after upgrading. Stable harness `latest` is **0.1.2** and includes
the shared TypeScript judge and automatic credential handoff.
