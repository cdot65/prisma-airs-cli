...
The **0.1.2-alpha.5.mcp.1** harness preview includes CLI **7.1.5** and the updated
...
is required; the bundled skill uses its owning environment's saved key. Approve
the specific live judge command when AIRS asks for credential-store and network
access. Dry runs remain sandboxed and need no key.
