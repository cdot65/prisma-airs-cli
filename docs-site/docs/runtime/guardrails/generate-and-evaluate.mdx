...
The CLI provides atomic commands for each step. An AI coding agent can orchestrate the full loop autonomously by following the protocol in `AGENTS.md`.
...
Run `airs-cli runtime topics sample` to see an example:
...
```bash
claude "Follow AGENTS.md to optimize a block topic called 'Fraud Detection' \
...
<TabItem value="codex-copilot" label="Codex / Copilot">

```bash
codex "Read .github/copilot-instructions.md, then follow AGENTS.md to optimize \
...
```bash
claude "Follow AGENTS.md to optimize an allow topic called 'Houston Astros' \
...
When you hand the loop to an AI coding agent, it follows the protocol defined in `AGENTS.md`:
...
- [`AGENTS.md`](https://github.com/cdot65/prisma-airs-cli/blob/main/AGENTS.md) — the agent optimization protocol
