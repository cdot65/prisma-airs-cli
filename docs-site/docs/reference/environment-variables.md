...
[`airs-cli tenant`](../cli/tenant.md). Every `PANW_*` variable from earlier releases, the
`PRISMA_AIRS_CONFIG_PATH` override, and a `.env` file in the working directory are ignored; run
`airs-cli doctor` to list any that are still set in your shell.
...
The SDK reads these on every request. The CLI honors them as-is; `airs-cli doctor` mentions when
they are on.

| Variable | Default | What it does |
|----------|---------|-------------|
| `PANW_AI_SEC_DEBUG` | off | Redacted request/response logging from the SDK; prefer `airs-cli --debug`, which writes a private JSONL file |
...
| `PANW_MGMT_CLIENT_ID`, `PANW_MGMT_CLIENT_SECRET`, `PANW_MGMT_TSG_ID` | `airs-cli tenant create <name>` prompts for them; rotate with `airs-cli tenant set <name> mgmtClientSecret` |
