...
The independently installed CLI **4.3.0**, with SDK **0.24.0**, passed **54/54** read-only checks at **2026-09-07T09:07:04.718Z**: installed-version verification, an owned positive control and 13 positive/empty-cohort pairs for each of four charts. Every pair runs the public CLI executable.
...
The separately installed CLI 4.3.0 passed **3/3** read-only checks at **2026-09-07T09:05:49.581Z**, including actual JSON and YAML commands with SDK 0.24.0. No traffic or key was created to populate the empty window.
