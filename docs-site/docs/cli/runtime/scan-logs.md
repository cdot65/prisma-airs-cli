...
CLI **5.0.0** deliberately makes `airs-cli runtime scan-logs query` exit **1** with migration
...
airs-cli runtime sessions list --interval 1 --unit day --output json
airs-cli runtime sessions list --interval 1 --unit day --all --output json
