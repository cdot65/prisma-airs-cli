...
No tests in this repo hit a live API. For live AIRS coverage, run the manual checklist in [Live Smoke Tests](smoke-tests.md) — that's the authoritative way to catch backend wire-format drift, especially under SDK 0.8.0's now-on runtime Zod validation.
