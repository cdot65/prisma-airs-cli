...
airs-cli runtime topics create --name "Pokemon" \
  --description "Pokemon discussions" --examples "Pikachu evolution" "Pokemon battle strategy"
airs-cli runtime topics apply --profile "Custom Topics Test" --name "Pokemon" --intent block

# Evaluate against a prompt set (CSV: prompt, expected, intent columns)
airs-cli runtime topics eval --profile "Custom Topics Test" --prompts pokemon-prompts.csv --topic "Pokemon" --output json
```
...
```bash
airs-cli redteam report <jobId>
```

Add `--attacks` to see individual prompt outcomes:

```bash
airs-cli redteam report <jobId> --attacks
```
...
3. **Re-evaluate** with `airs-cli runtime topics eval` using updated prompts and iterate
4. **Revert a topic** with `airs-cli runtime topics revert` to roll back a bad change
5. **Abort a running scan** if needed: `airs-cli redteam abort <jobId>`
...
airs-cli runtime topics create --name "$TOPIC" \
  --description "Pokemon discussions" --examples "Pikachu evolution" "Pokemon battle strategy"
airs-cli runtime topics apply --profile "$PROFILE" --name "$TOPIC" --intent block

# 2. Evaluate (iterate as needed)
airs-cli runtime topics eval --profile "$PROFILE" --prompts pokemon-prompts.csv --topic "$TOPIC" --output json
...
airs-cli redteam status "$JOB_ID"

# 6. View report with per-prompt details
airs-cli redteam report "$JOB_ID" --attacks
```
